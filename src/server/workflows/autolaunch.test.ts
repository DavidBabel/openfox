...
describe('favorite-workflow auto-launch timer', () => {
  ...
  it('schedules, broadcasts the deadline, and fires after 60s', () => {
    ...
    expect(Number(first.payload['deadline'])).toBeCloseTo(Date.now() + AUTO_LAUNCH_DELAY_MS, -2)

    vi.advanceTimersByTime(AUTO_LAUNCH_DELAY_MS - 1)
  ...
  it('cancels before expiry and never fires', () => {
    ...
    expect(hasAutoLaunchPending('s1')).toBe(false)
    vi.advanceTimersByTime(AUTO_LAUNCH_DELAY_MS + 1000)
    expect(fireCalls).toHaveLength(0)
  ...
  it('ignores a second schedule while one is pending', () => {
    ...
    vi.advanceTimersByTime(AUTO_LAUNCH_DELAY_MS)
  ...
  it('reports remaining countdown for reconnect sync', () => {
    ...
    expect(Number(msg!.payload['deadline'])).toBeCloseTo(Date.now() + AUTO_LAUNCH_DELAY_MS - 20_000, -3)
